...
StatusOr<uint32_t> primary_compaction_score_by_policy(TabletManager* tablet_mgr,
    ...
    ASSIGN_OR_RETURN(auto picked_rowsets, policy.pick_rowsets(metadata, true, &has_dels, 0 /* max_segments */));
    uint32_t segment_num_score = 0;
    for (int i = 0; i < picked_rowsets.size(); i++) {
        const bool has_del = has_dels[i];
        auto current_score = picked_rowsets[i].stat.num_segments;
        if (has_del) {
    ...
    if (selected_level->segment_num >= min_compaction_segment_num) {
        ...
        for (auto i : selected_level->rowsets) {
            ...
            if (segment_num_score + cur_segment_score >= max_segments) {
                if (restrict_segment_count) {
                    segment_limit = std::min((size_t) max_segments - segment_num_score, cur_segment_score);
                    LOG(INFO) << "partial compaction for tablet " << _tablet_metadata->id()
                              << ", max: " << max_segments << ", total: " << cur_segment_score
                              << ", limit: " << segment_limit;
                }
                need_break = true;
            }
